...
class DeployForgeGUI:
    ...
    def _create_builder_tab(self, parent: ttk.Frame):
        ...
        ttk.Entry(source_frame, textvariable=self.image_path, width=50).grid(
            row=0, column=1, pady=5, padx=5
        )
        ttk.Button(source_frame, text="Browse...", command=self.select_image).grid(
        ...
        description_label = ttk.Label(
            profile_frame, text=profile_descriptions["gamer"], wraplength=600, foreground="gray"
        )
        ...
        ttk.Button(output_frame, text="Browse...", command=self.select_output).grid(
            row=0, column=2, pady=5
        )
        ...
        ttk.Checkbutton(
            options_frame, text="Validate after build", variable=self.validate_option
        ).grid(row=0, column=0, sticky=tk.W)
        ttk.Checkbutton(
            options_frame, text="Generate analysis report", variable=self.report_option
        ...
        ttk.Button(
            button_frame, text="Build Image", command=self.build_image, style="Accent.TButton"
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Cancel", command=self.cancel_build, state=tk.DISABLED).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _create_profiles_tab(self, parent: ttk.Frame):
        ...
        ttk.Label(list_frame, text="Available Profiles:", font=("Arial", 10, "bold")).pack(
            anchor=tk.W, pady=5
        )
        ...
        for name, description in profiles_data:
            ...
            ttk.Label(profile_frame, text=name, font=("Arial", 10, "bold")).pack(
                anchor=tk.W, padx=10, pady=5
            )
            ttk.Label(profile_frame, text=description, foreground="gray").pack(
                anchor=tk.W, padx=10, pady=5
            )
            ...
            ttk.Button(
                btn_frame, text="View Details", command=lambda n=name: self.view_profile(n)
            ).pack(side=tk.LEFT, padx=2)
            ttk.Button(
                btn_frame, text="Apply", command=lambda n=name: self.apply_profile_quick(n)
            ).pack(side=tk.LEFT, padx=2)
    ...
    def _create_presets_tab(self, parent: ttk.Frame):
        ...
        ttk.Button(toolbar, text="Create Preset", command=self.create_preset).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(toolbar, text="Import Preset", command=self.import_preset).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _create_settings_tab(self, parent: ttk.Frame):
        ...
        ttk.Entry(general_frame, textvariable=python_path, width=40).grid(
            row=0, column=1, pady=5, padx=5
        )
        ...
        ttk.Combobox(
            general_frame,
            textvariable=default_profile,
            values=["gamer", "developer", "enterprise"],
            width=37,
        ...
        ttk.Checkbutton(advanced_frame, text="Auto-validate images after build").grid(
            row=0, column=0, sticky=tk.W
        )
        ttk.Checkbutton(advanced_frame, text="Generate analysis reports automatically").grid(
            row=1, column=0, sticky=tk.W
        )
        ttk.Checkbutton(advanced_frame, text="Check for updates on startup").grid(
    ...
    def _create_log_tab(self, parent: ttk.Frame):
        ...
        ttk.Button(
            toolbar, text="Clear Log", command=lambda: self.log_text.delete(1.0, tk.END)
        ).pack(side=tk.LEFT, padx=5)
    ...
    def _create_status_bar(self):
        ...
        ttk.Label(status_bar, textvariable=self.status_text, anchor=tk.W).pack(
    ...
    def save_log(self):
        """Save log to file."""
        filename = filedialog.asksaveasfilename(
            title="Save Log", defaultextension=".txt", filetypes=[("Text Files", "*.txt")]
        )
