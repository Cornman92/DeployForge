...
class SetupWizard:
    ...
    PRESETS: Dict[str, WizardConfig] = {
        'gaming': WizardConfig(
            ...
            recommended_apps=['Epic Games Launcher', 'GOG Galaxy', 'GeForce Experience', 'MSI Afterburner'],
        ...
        for preset_name in presets_to_include:
            if preset_name in self.PRESETS:
                ...
                wizard_data['presets'][preset_name] = {
                    ...
                    'apps_count': len(preset_config.essential_apps) + len(preset_config.recommended_apps),
                    'requirements': {
...
def create_quick_setup(preset: str = 'gaming') -> Dict[str, Any]:
    ...
    """
    wizard = SetupWizard()
    preset_enum = SetupPreset(preset) if preset in [p.value for p in SetupPreset] else SetupPreset.HOME_USER
